    ...
    <section className="min-h-screen py-32 px-8 lg:px-24 relative z-10 flex flex-col justify-center">
      ...
      >
        ...
        <div className="glass-panel-light p-8 rounded-2xl border-l-8 border-l-[#8B4513] space-y-4">
          ...
          <p className="text-xl md:text-2xl text-[#5C4033] leading-relaxed text-justify">{overview}</p>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="space-y-4 mb-6">
          {nature.paragraphs.map((paragraph) => (
            <p key={paragraph} className="text-xl md:text-2xl text-[#2C1E16] leading-relaxed text-justify">
              {paragraph}
            </p>
          ))}
        </div>
        <div className="grid gap-4 md:grid-cols-3">
          {nature.items.map((item) => (
            <div key={item} className="rounded-2xl bg-white/55 border border-white/50 p-5 text-xl md:text-2xl text-[#4A3219] leading-relaxed">
              {item}
            </div>
          ))}
        </div>
