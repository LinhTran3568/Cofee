export const presentationData = {
  hero: {
    title: "Lợi ích kinh tế và",
    highlightTitle: "Quan hệ lợi ích kinh tế",
    subtitle: "Từ vườn cà phê Đắk Lắk đến ly cà phê 85.000đ tại Highlands",
  },
  story: {
    intro: {
      title: "Câu chuyện mở đầu",
      lead:
        "Anh Khánh thức dậy lúc 5 giờ sáng ở Đắk Lắk, hái từng hạt cà phê rồi bán cho thương lái với giá 40.000đ/kg. Cùng ngày hôm đó, tại TP.HCM, một ly cà phê từ chính hạt đó được Highlands bán với giá 85.000đ.",
      insight:
        "Người trồng ra hạt cà phê nhận phần nhỏ nhất, trong khi bỏ ra nhiều năm lao động và gánh chịu rủi ro lớn nhất trong toàn chuỗi.",
      stats: [
        { label: "Giá bán nguyên liệu", value: "40.000đ/kg" },
        { label: "Giá bán lẻ", value: "85.000đ/ly" },
        { label: "Thực trạng", value: "Nông dân nhận ít nhất" },
      ],
    },
    farmer: {
      name: "Anh Khánh (Nông dân Đắk Lắk)",
      description:
        "Nông dân (anh Khánh): Thu nhập từ lao động. Bán 40.000đ/kg, đủ tái sản xuất tối thiểu.",
      image: "/assets/raw_beans.png",
    },
    retail: {
      name: "Highlands Coffee (Bán lẻ)",
      description:
        "Highlands Coffee: Lợi nhuận từ thương hiệu. Bán 85.000đ/ly, nhận phần lớn nhất trong chuỗi.",
      image: "/assets/premium_cup.png",
    },
  },
  concept: {
    title: "I. Khái niệm lợi ích kinh tế",
    subtitle: "",
    definition:
      "Để tồn tại và phát triển, con người cần được thỏa mãn các nhu cầu vật chất. Lợi ích kinh tế là lợi ích vật chất, lợi ích thu được khi thực hiện các hoạt động kinh tế của con người.",
    overview:
      "Trong nền kinh tế thị trường, mỗi chủ thể hành động vì lợi ích riêng của mình:",
    actorsIntro: "Chủ thể - Lợi ích kinh tế - Biểu hiện trong chuỗi cà phê",
    actors: [
      {
        subject: "Nông dân (anh Khánh)",
        benefit: "Thu nhập từ lao động",
        expression: "Bán 40.000đ/kg — đủ tái sản xuất tối thiểu",
      },
      {
        subject: "Thương lái",
        benefit: "Lợi nhuận từ trung gian",
        expression: "Hưởng chênh lệch giữa giá mua và giá bán",
      },
      {
        subject: "Nhà rang xay",
        benefit: "Lợi nhuận từ chế biến",
        expression: "Tạo giá trị gia tăng qua kỹ thuật rang xay",
      },
      {
        subject: "Highlands Coffee",
        benefit: "Lợi nhuận từ thương hiệu",
        expression: "Bán 85.000đ/ly — nhận phần lớn nhất chuỗi",
      },
    ],
    nature: {
      title: "II. Bản chất và biểu hiện",
      quote:
        'Ph. Ăngghen chỉ rõ: "những quan hệ kinh tế của một xã hội nhất định biểu hiện trước hết dưới hình thái lợi ích".',
      paragraphs: [
        "1. Bản chất",
        "Lợi ích kinh tế phản ánh mục đích và động cơ của các quan hệ giữa các chủ thể trong nền sản xuất xã hội.",
        "Trong chuỗi cà phê, bản chất đó thể hiện rõ:",
      ],
      items: [
        "Anh Khánh canh tác, mục tiêu là đảm bảo thu nhập nuôi gia đình.",
        "Thương lái thu gom, mục tiêu là khai thác chênh lệch thông tin giá.",
        "Highlands vận hành chuỗi, mục tiêu là tối đa hóa lợi nhuận thương hiệu.",
      ],
    },
    dimensions: [
      {
        title: "Quan hệ chiều dọc",
        desc:
          "Là sự liên kết từ Anh Khánh (Nông dân) → Thương lái → Nhà rang xay → Highland Coffee (Bán lẻ). Trong đó, Highland ở vị thế cao nhất, kiểm soát luật chơi của cả chuỗi.",
      },
      {
        title: "Quan hệ chiều ngang",
        desc:
          "Là sự cạnh tranh giữa anh Khánh với các hộ nông dân khác tại Đắk Lắk, hoặc giữa Highland với các đối thủ như Starbucks, Trung Nguyên.",
      },
    ],
    relations: {
      theory:
        "2. Bản chất của quan hệ: sự thống nhất và mâu thuẫn. Giáo trình khẳng định quan hệ lợi ích luôn có hai mặt song hành:",
      unity:
        "Anh Khánh cần Highlands để tiêu thụ sản phẩm; Highlands cần anh Khánh để có nguồn nguyên liệu đầu vào. Nếu một bên gặp sự cố, bên kia cũng sẽ bị ảnh hưởng trực tiếp.",
      conflict:
        'Số liệu thực tế: Anh Khánh bán cà phê nhân với giá 40.000đ/kg, trong khi Highland bán ra 85.000đ/ly. Một kg cà phê pha được khoảng 40-50 ly, tạo ra doanh thu hàng triệu đồng. Người nông dân chỉ nhận được phần nhỏ nhất trong tổng giá trị.\n\nBiểu hiện mâu thuẫn: Các doanh nghiệp trong chuỗi cung ứng như Highland thường ký hợp đồng cung ứng dài hạn hoặc áp dụng các cơ chế quản trị rủi ro về giá với nhà cung cấp để giảm tác động của biến động giá nguyên liệu. Khi giá thế giới giảm, anh Khánh phải chịu lỗ trực tiếp, trong khi Highland vẫn duy trì được lãi. Đây là mâu thuẫn điển hình khi các chủ thể tối ưu hóa lợi ích riêng bằng những phương thức đối lập.',
    },
    roleTitle: "III. Vai trò của lợi ích kinh tế",
    roles: [
      {
        title: "Là động lực trực tiếp",
        desc:
          "Lợi ích kinh tế là động lực trực tiếp thúc đẩy mọi hoạt động kinh tế. Khi lợi ích không được đảm bảo, động lực sẽ mất đi. Khi giá cà phê thế giới giảm mạnh giai đoạn 2012-2013, hàng nghìn nông dân Tây Nguyên bỏ vườn. Không phải vì lười biếng, mà vì lợi ích kinh tế không còn đủ để duy trì động lực canh tác.",
      },
      {
        title: "Là cơ sở phát triển các lợi ích khác",
        desc:
          'C. Mác khẳng định: "Cội nguồn phát triển của xã hội không phải là quá trình nhận thức, mà là các quan hệ của đời sống vật chất, tức là các lợi ích kinh tế". Với anh Khánh, điều này có nghĩa là: thu nhập ổn định mới có thể đầu tư giáo dục cho con cái; thu nhập ổn định mới có điều kiện chăm lo sức khỏe, y tế; còn khi thu nhập bấp bênh thì toàn bộ các lợi ích khác đều bị kéo xuống theo.',
      },
    ],
    roleQuote:
      '"Nước độc lập mà dân không được hưởng ấm no, hạnh phúc thì độc lập cũng không có ý nghĩa gì." - Hồ Chí Minh',
    deepDiveTitle: "IV. Quan hệ lợi ích kinh tế",
    deepDiveSubtitle:
      "Phân tích trường hợp chuỗi giá trị cà phê từ người trồng đến doanh nghiệp bán lẻ",
    deepDiveIntroTitle: "1. Khái niệm và không gian tương tác của quan hệ lợi ích",
    deepDiveIntroTheory:
      "Quan hệ lợi ích kinh tế là sự thiết lập những tương tác giữa các chủ thể nhằm mục tiêu xác lập các lợi ích kinh tế. Quan hệ này biểu hiện qua hai chiều: chiều dọc và chiều ngang.",
    deepDiveIntroApply: "Áp dụng bài toán thực tế:",
  },
  valueChain: {
    title: "2. Biểu hiện - ai nhận được gì?",
    subtitle:
      "Mỗi chủ thể nhận lợi ích tương ứng với vai trò của mình. Tuy nhiên, sự phân phối hiện tại không phản ánh đúng đóng góp lao động thực tế:",
    steps: [
      {
        id: 1,
        name: "Nông dân",
        value: "3-5 năm / vụ",
        effort: "Rủi ro cao nhất",
        role: "Lao động đầu tư",
        note: "Yếu nhất - phụ thuộc thương lái",
      },
      {
        id: 2,
        name: "Thương lái",
        value: "1-2 ngày / chuyến",
        effort: "Rủi ro trung bình",
        role: "Lao động đầu tư",
        note: "Có lợi thế thông tin",
      },
      {
        id: 3,
        name: "Nhà rang xay",
        value: "2-3 ngày / lô",
        effort: "Rủi ro trung bình",
        role: "Lao động đầu tư",
        note: "Tạo giá trị gia tăng thực",
      },
      {
        id: 4,
        name: "Highlands Coffee",
        value: "Vận hành liên tục",
        effort: "Rủi ro thấp nhất",
        role: "Lao động đầu tư",
        note: "Mạnh nhất - kiểm soát thương hiệu",
      },
    ],
    commentary:
      "Qua đó có thể thấy người nông dân là chủ thể đầu tiên của chuỗi giá trị nhưng lại ở vị thế yếu nhất. Họ đầu tư lao động dài hạn, chịu rủi ro lớn nhất về giá thế giới, thời tiết và sâu bệnh, nhưng phần lợi ích nhận được vẫn thấp hơn rõ rệt so với các khâu sau.",
    update2024:
      "Điểm mấu chốt là Highlands ký hợp đồng giá cố định từ trước, qua đó chuyển phần lớn rủi ro biến động giá về phía nông dân. Khi giá thế giới giảm, anh Khánh là người lỗ trước; còn Highlands vẫn có khả năng duy trì lợi nhuận.",
    ending:
      "Sự chênh lệch này cho thấy lợi ích kinh tế không chỉ phụ thuộc vào công sức lao động, mà còn phụ thuộc vào vị thế của từng chủ thể trong chuỗi và khả năng kiểm soát khâu có giá trị gia tăng cao.",
  },
  causes: {
    title: "3. Các nhân tố ảnh hưởng đến quan hệ lợi ích",
    subtitle:
      "Có 4 nhân tố cốt lõi lý giải vì sao trong quan hệ này, phần lợi ích lại nghiêng mạnh về phía doanh nghiệp:",
    items: [
      {
        title: "Trình độ phát triển của lực lượng sản xuất",
        desc:
          "Anh Khánh canh tác thủ công, quy mô nhỏ lẻ, năng suất thấp dẫn đến giá thành cao nhưng giá trị gia tăng lại thấp nhất chuỗi.",
      },
      {
        title: "Địa vị của chủ thể trong hệ thống quan hệ sản xuất",
        desc:
          "Anh Khánh là hộ nông dân đơn lẻ, không có tổ chức đại diện như hợp tác xã nên sức đàm phán rất thấp. Ngược lại, Highlands nắm giữ thương hiệu và kênh phân phối nên có lợi thế áp đặt giá bán.",
      },
      {
        title: "Chính sách phân phối thu nhập",
        desc:
          "Hiện nay, việc thiếu các cơ chế bảo vệ như giá sàn thu mua khiến nông dân hoàn toàn bị động và chịu thiệt thòi trước sự biến động của thị trường tự do.",
      },
      {
        title: "Hội nhập kinh tế quốc tế",
        desc:
          "Giá cà phê của anh Khánh phụ thuộc trực tiếp vào sàn giao dịch ICE London. Các tập đoàn lớn như Highlands có các công cụ tài chính để phòng ngừa rủi ro, còn người nông dân phải gánh chịu trực tiếp mọi cú sốc từ thị trường thế giới.",
      },
    ],
  },
  stateRole: {
    title: "V. Vai trò của Nhà nước trong đảm bảo hài hòa quan hệ lợi ích",
    subtitle:
      "Kinh tế thị trường không tự bảo đảm được sự hài hòa vì lợi ích của các chủ thể luôn mâu thuẫn nhau. Nhà nước cần can thiệp để điều tiết và giải quyết xung đột.",
    context:
      "Trong chuỗi giá trị cà phê, lợi ích đang bị lệch pha nghiêm trọng. Chuỗi bán lẻ hưởng lợi lớn nhất nhờ nắm giữ vốn, thương hiệu và khả năng chuyển rủi ro. Ngược lại, người nông dân bỏ nhiều công sức, đối mặt thiên tai trực tiếp nhưng nhận phần lợi ích nhỏ nhất. Thị trường tự do không tự giải quyết được bất công này.",
    foundationTitle: "2. Bảo vệ lợi ích hợp pháp, tạo môi trường thuận lợi",
    foundationLead:
      "Nhà nước cần kiến tạo môi trường kinh doanh thông qua ổn định chính trị, pháp luật thông thoáng, chính sách vĩ mô phù hợp và hạ tầng đồng bộ.",
    foundation: [
      "Để bảo vệ những người yếu thế như anh Khánh trước các tập đoàn lớn, Nhà nước cần tạo hành lang pháp lý vững chắc và khuyến khích mô hình hợp tác xã kiểu mới.",
      "Khi các hộ nông dân liên kết lại thành một pháp nhân có tổ chức, vị thế đàm phán giá và khả năng ký kết hợp đồng với doanh nghiệp sẽ được nâng cao, không còn dễ bị ép giá.",
    ],
    redistributionTitle: "3. Điều hòa lợi ích Cá nhân - Doanh nghiệp - Xã hội",
    redistributionTheory:
      "Thị trường tự do có thể làm gia tăng phân hóa giàu nghèo, vì vậy Nhà nước phải sử dụng chính sách phân phối lại để điều tiết. Giải pháp gốc rễ vẫn là phát triển lực lượng sản xuất và khoa học - công nghệ để nâng thu nhập chung.",
    redistributionItems: [
      "Trước mắt, Nhà nước cần triển khai bảo hiểm nông nghiệp, trợ giá hoặc lập quỹ bình ổn giá cà phê để hỗ trợ nông dân khi thị trường lao dốc.",
      "Về lâu dài, Nhà nước cần đầu tư hạ tầng logistics và chuyển giao công nghệ chế biến sâu tại Đắk Lắk để nông dân có thể tham gia vào những khâu có giá trị gia tăng cao hơn.",
    ],
    preventive: {
      title: "4. Kiểm soát, ngăn ngừa các tác động tiêu cực",
      desc:
        "Nhà nước cần kiểm soát và ngăn ngừa các quan hệ lợi ích có ảnh hưởng tiêu cực đến sự phát triển xã hội. Để hạn chế tình trạng ép giá và chèn ép người yếu thế, có thể triển khai ba biện pháp cụ thể sau:",
      items: [
        {
          title: "Minh bạch thông tin giá",
          desc:
            "Nhà nước cần đẩy mạnh hoạt động của sàn giao dịch cà phê và các công cụ cập nhật giá thị trường thế giới theo thời gian thực. Khi nông dân nắm được giá thực tế, thương lái sẽ khó lợi dụng chênh lệch thông tin để ép giá.",
        },
        {
          title: "Gia tăng giá trị qua chứng nhận quốc tế",
          desc:
            "Nhà nước có thể hỗ trợ các hợp tác xã đạt chuẩn Fair Trade hoặc 4C để nâng giá trị sản phẩm và tạo thêm phần thu nhập trực tiếp cho nông dân.",
        },
        {
          title: "Hỗ trợ nền tảng sản xuất",
          desc:
            "Thông qua các chương trình tái canh cà phê, Nhà nước có thể hỗ trợ vốn và kỹ thuật để thay vườn già cỗi, giảm giá thành, tăng năng suất và cải thiện biên lợi nhuận cho nông dân.",
        },
      ],
    },
    resolution: {
      title: "5. Giải quyết mâu thuẫn trong quan hệ lợi ích kinh tế",
      desc:
        "Mâu thuẫn lợi ích kinh tế là khách quan. Khi những mâu thuẫn này bùng phát thành khủng hoảng, Nhà nước phải trực tiếp can thiệp để giải quyết nhằm tránh bất ổn xã hội.",
      actions: [
        "Bài học từ giai đoạn 2012-2013 cho thấy khi giá cà phê giảm sâu, nhiều nông dân đã chặt bỏ vườn cây, làm đứt gãy nguồn cung. Điều đó chứng minh cần giải quyết mâu thuẫn ngay từ sớm.",
        "Giải pháp phù hợp là cơ chế đàm phán ba bên gồm hợp tác xã đại diện nông dân, doanh nghiệp hoặc thương lái, và cơ quan quản lý nhà nước.",
        "Mục tiêu là các bên cùng nhân nhượng, chia sẻ rủi ro và đặt lợi ích chung lên trên lợi ích cục bộ.",
      ],
    },
  },
  conclusion: {
    title: "VI. KẾT LUẬN",
    message:
      "Câu chuyện của anh Khánh là biểu hiện điển hình của sự phân phối lợi ích kinh tế không tương xứng với đóng góp thực tế trong nền kinh tế thị trường.",
    summary:
      "Thông qua trường hợp này, có thể thấy quan hệ lợi ích kinh tế không chỉ là quan hệ mua bán đơn thuần, mà còn phản ánh vị thế, quyền lực và khả năng chi phối của từng chủ thể trong toàn bộ chuỗi giá trị.",
    bridge:
      "Chỉ khi lợi ích cá nhân được tôn trọng và bảo đảm công bằng thì lợi ích kinh tế mới thực sự trở thành động lực phát triển. Ngược lại, nếu sự bất bình đẳng kéo dài, động lực sản xuất sẽ bị triệt tiêu và dễ dẫn đến bất ổn xã hội.",
    quotes: [
      "Vì vậy, để bảo đảm công bằng hơn cho những người sản xuất như anh Khánh, cần phát huy vai trò điều tiết của Nhà nước, nâng cao vị thế đàm phán của nông dân và hướng tới sự hài hòa lợi ích giữa cá nhân, doanh nghiệp và xã hội.",
      '"Coi lợi ích kinh tế là động lực của các hoạt động kinh tế; phải tôn trọng lợi ích cá nhân chính đáng." - Văn kiện Đại hội XII, ĐCSVN',
    ],
    buttonText: "Quay lại mở đầu",
    finalWish: [
      "Từ hạt cà phê đến ly cà phê,",
      "mỗi mắt xích đều góp phần tạo nên giá trị.",
      "",
      "Nhưng giá trị chỉ thực sự bền vững",
      "khi lợi ích được phân phối công bằng.",
    ]
  },
};
